...
// ひらがな→カタカナ変換(Unicodeのひらがなブロックはカタカナブロックと
// +0x60の固定オフセットで対応している)。Googleフォームのふりがな列は
// ひらがな入力が多いため、スプレッドシート書き出し用にカタカナへ揃える。
// 漢字や英字はそのまま素通りする(このスクリプトは「読みの変換」であって
// 「漢字からの読み推定」はできないため、ふりがな列が無い媒体では使わない)。
