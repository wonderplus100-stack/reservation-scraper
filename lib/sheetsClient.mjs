...
export const OFFICIAL_EVENTS_SHEET = "OfficialEvents";
...
// 指定した名前のシートタブが無ければ作成する。values.clear/updateは
// 既存のシートタブが無いと"Unable to parse range"で失敗するため、
// OfficialEventsのような新規シートを初回実行時に自動生成できるようにする。
...
// OfficialEvents: Wonder+公式HP(の裏側のスケジュール表)から取り込んだ、
// 「本来あるべきイベント一覧」。予約が1件も無いイベントもダッシュボードの
// プルダウンに出すための正となるリスト。毎回スケジュール表から作り直す
// スナップショットなので、他のスナップショット系シートと同じくreplaceする。
const OFFICIAL_EVENTS_HEADERS = ["canonicalEventId", "canonicalEventName", "month", "day", "venue", "venueKey", "time", "capacity"];

export async function readOfficialEvents(sheetId) {
  return readSheetAsObjects(sheetId, OFFICIAL_EVENTS_SHEET);
}

export async function replaceOfficialEvents(sheetId, events) {
  const values = events.map((event) => OFFICIAL_EVENTS_HEADERS.map((key) => event[key] ?? ""));
  await replaceSheet(sheetId, OFFICIAL_EVENTS_SHEET, OFFICIAL_EVENTS_HEADERS, values);
}

// UpdateControl: ダッシュボードの「今すぐ更新」ボタン(Code.js)と、
// PC側の常駐ポーリングスクリプト(peatix-update-watcher.mjs)が同じ
// スプレッドシートの1行を介してやり取りするための簡易な指示・状態欄。
// GAS側はSpreadsheetAppで直接同じシートを読み書きするため、専用のAPIは
// 不要(ポーリング間隔が数秒〜十数秒と粗いため、厳密な排他制御はしない)。
export const UPDATE_CONTROL_SHEET = "UpdateControl";
const UPDATE_CONTROL_HEADERS = ["status", "message", "requestedAt", "updatedAt"];

export async function readUpdateControl(sheetId) {
  const rows = await readSheetAsObjects(sheetId, UPDATE_CONTROL_SHEET).catch(() => []);
  if (rows.length === 0) {
    return { status: "idle", message: "", requestedAt: "", updatedAt: "" };
  }
  return rows[0];
}

export async function writeUpdateControl(sheetId, { status, message = "", requestedAt }) {
  const sheets = await getSheetsApi();
  await ensureSheetExists(sheets, sheetId, UPDATE_CONTROL_SHEET);
  const current = await readUpdateControl(sheetId);
  const row = {
    status,
    message,
    requestedAt: requestedAt !== undefined ? requestedAt : current.requestedAt || "",
    updatedAt: new Date().toISOString()
  };
  const values = [UPDATE_CONTROL_HEADERS, UPDATE_CONTROL_HEADERS.map((key) => row[key] ?? "")];
  await sheets.spreadsheets.values.clear({ spreadsheetId: sheetId, range: UPDATE_CONTROL_SHEET });
  await sheets.spreadsheets.values.update({
    spreadsheetId: sheetId,
    range: `${UPDATE_CONTROL_SHEET}!A1`,
    valueInputOption: "RAW",
    requestBody: { values }
  });
}
