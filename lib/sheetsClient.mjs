...
// シート名だけをrangeとして渡す場合、A1記法ではスペースなどを含む名前を
// シングルクォートで囲む必要がある(例: 「フォームの回答 1」→ 'フォームの回答 1')。
// これを忘れると Sheets API が "Unable to parse range" エラーを返す。
...
export async function readSheetAsObjects(spreadsheetId, sheetName) {
  const values = await readSheetValues(spreadsheetId, quoteSheetName(sheetName));
